...
lblLine = tk.Label(clientFrame, text="Chatters!!").pack()
...
def start_server():
    ...
    try:
        server.bind((HOST_ADDR, HOST_PORT)) # this causes error because the socket does not close due to the button
...
def stop_server():
    ...
    server = server
    server.close()
    # think I need to close the threads as well.

def accept_clients(the_server, y):
    while True:
        try: # this is a temp fix for my issue
            client, addr = the_server.accept()
            clients.append(client)
            # use a thread so as not to clog the gui thread
            threading.Thread(target=send_receive_client_message, args=(client, addr)).start()
        except ConnectionAbortedError as e:
            print("The socket did not close")
